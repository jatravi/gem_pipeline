...
INCLUDE_KEYWORDS = {
    line.strip().lower()
    for line in (
        BASE_DIR / "configs" / "keyword_include.txt"
...
EXCLUDE_KEYWORDS = {
    line.strip().lower()
    for line in (BASE_DIR / "configs" / "keyword_exclude.txt").read_text(
        encoding="utf-8"
    ).splitlines()
    if line.strip()
}
...
def score_bid_text(title: str | None, description: str | None = None) -> KeywordFilterResult:
...
def is_bid_relevant_by_keywords(title: str | None, description: str | None = None) -> bool:
